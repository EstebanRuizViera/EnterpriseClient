...
class AvailabilityRepository(application: Application) {
    private val availabilityDao: AvailabilityDao? =
        ReservationDatabase.getInstance(application)?.availabilityDao()
    ...
    fun getAllAvailability(): List<Availability>? {
        ...
        if (availabilityDao != null)
            return GetAllAvailabilityrAsyncTask(availabilityDao).execute().get()
        return null
    }
    ...
    private class GetAllAvailabilityrAsyncTask(private val availabilityDao: AvailabilityDao) :
        AsyncTask<Void, Void, List<Availability>>() {
        override fun doInBackground(vararg ids: Void?): List<Availability>? {
            var availability = availabilityDao.getAllAvailability()
            if (availability != null) {
                return availability
            }
